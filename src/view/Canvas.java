...
public class Canvas extends JComponent implements Observer {
    ...
    public static final Stroke GRID_STROKE = 
            new BasicStroke(1, BasicStroke.CAP_SQUARE, BasicStroke.JOIN_ROUND, 1, new float[] {5}, 0);
    ...
    public Stroke createStroke(Strokes type, float thickness) {     
        switch (type) {
            case SOLID: return new BasicStroke(thickness);

            case DASHED: 
                return new BasicStroke(thickness,BasicStroke.CAP_SQUARE,
                                       BasicStroke.JOIN_BEVEL,1,new float[] {5f}, 0) ;
            case DOTTED: return new BasicStroke(thickness,BasicStroke.CAP_SQUARE,
                                     BasicStroke.JOIN_BEVEL,1,new float[] {.5f,10f}, 0) ;
            case DASH_AND_DOT: return new BasicStroke(thickness,BasicStroke.CAP_SQUARE,
                                    BasicStroke.JOIN_BEVEL,1,new float[] {.5f,10,7,10}, 0) ;
            case DOUBLE_LINE:  return new CompositeStroke(4, 1);
        }
        return null;
    }
}
